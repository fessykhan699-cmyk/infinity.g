...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed w-full z-50 glass-nav top-0 left-0 border-b border-white/5 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <a href="/#contact" className="px-6 py-2.5 rounded-full bg-white text-black text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-white transition-all shadow-[0_10px_30px_rgba(255,255,255,0.1)]">Get Started</a>
        </div>

        <div className="flex items-center gap-4">
          <a href="mailto:hello@infinitydigital.ae" className="w-10 h-10 rounded-full border border-slate-700 flex items-center justify-center hover:bg-white/5 transition-colors">
            <span className="material-icons-outlined text-slate-300 text-lg">mail</span>
          </a>
          <button 
            onClick={toggleMenu}
            className="md:hidden w-10 h-10 rounded-full border border-slate-700 flex items-center justify-center hover:bg-white/5 transition-colors relative"
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-background-dark/95 border-b border-white/5 py-10 px-8 animate-in slide-in-from-top duration-300 backdrop-blur-2xl">
          <div className="flex flex-col gap-8 max-w-7xl mx-auto">
            <a href="/" onClick={closeMenu} className="text-3xl font-display font-light hover:text-primary transition-colors flex items-center justify-between group">
              Home <span className="material-icons-outlined opacity-0 group-hover:opacity-100 transition-opacity">east</span>
            </a>
            <a href="/#services" onClick={closeMenu} className="text-3xl font-display font-light hover:text-primary transition-colors flex items-center justify-between group">
              Services <span className="material-icons-outlined opacity-0 group-hover:opacity-100 transition-opacity">east</span>
            </a>
            <a href="/#work" onClick={closeMenu} className="text-3xl font-display font-light hover:text-primary transition-colors flex items-center justify-between group">
              Showcase <span className="material-icons-outlined opacity-0 group-hover:opacity-100 transition-opacity">east</span>
            </a>
            <a href="/placeholders" onClick={closeMenu} className="text-3xl font-display font-light hover:text-primary transition-colors flex items-center justify-between group">
              Asset Library <span className="material-icons-outlined opacity-0 group-hover:opacity-100 transition-opacity">east</span>
            </a>
            <a href="/#contact" onClick={closeMenu} className="text-3xl font-display font-light hover:text-primary transition-colors flex items-center justify-between group">
