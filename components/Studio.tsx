...
const Studio: React.FC = () => {
  ...
  return (
    <section id="studio" className="py-24 md:py-48 px-4 max-w-6xl mx-auto relative">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[900px] h-[520px] bg-primary/10 rounded-full blur-[80px] pointer-events-none -z-10"></div>
      ...
      <ScrollReveal direction="up" className="text-center mb-16 md:mb-20">
        ...
        <p className="text-slate-200 text-base md:text-lg lg:text-xl font-light max-w-2xl mx-auto leading-relaxed">
      ...
      <div className="grid lg:grid-cols-12 gap-12 items-start mb-24">
        ...
        <ScrollReveal direction="left" className="lg:col-span-5 space-y-8">
          <div className="glass-card p-6 md:p-8 rounded-[2.5rem] border-white/20 reactive-glass space-y-8">
            {/* Model Selection */}
            ...
            <button 
              ...
              className="w-full py-5 min-h-[44px] rounded-2xl bg-primary text-white text-xs font-black uppercase tracking-[0.4em] hover:brightness-110 transition-all shadow-[0_20px_50px_-25px_rgba(0,0,0,0.7)] active:scale-[0.98] disabled:opacity-20"
            >
        ...
        <ScrollReveal direction="right" className="lg:col-span-7 h-full">
          <div className="glass-card rounded-[3rem] border-white/20 p-4 min-h-[520px] flex flex-col reactive-glass overflow-hidden group">
            <div className="relative flex-grow rounded-[2.5rem] overflow-hidden bg-white/[0.05] border border-white/10 flex items-center justify-center">
              {loading && (
              ...
              {error ? (
                <div className="absolute inset-0 z-30 flex flex-col items-center justify-center p-10 text-center bg-background-dark/90 backdrop-blur-lg animate-in fade-in duration-[700ms]">
                  ...
                  <div className="flex gap-4">
                    {error.type === 'auth' ? (
                       <button 
                        onClick={handleProToggle}
                        className="px-10 py-4 min-h-[44px] rounded-full bg-primary text-white text-[10px] font-black uppercase tracking-widest shadow-xl hover:brightness-110"
                      >
                        Re-authenticate Pro
                      </button>
                    ) : (
                      <button 
                        ...
                        className="px-10 py-4 min-h-[44px] rounded-full bg-white text-black text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-slate-200"
                      >
                        Retry Synthesis
                      </button>
                    )}
                    <button 
                      ...
                      className="px-10 py-4 min-h-[44px] rounded-full bg-white/10 border border-white/20 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/20"
                    >
                      Dismiss
                    </button>
                  </div>
                ...
                <>
                  ...
                  />
                  <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-4 opacity-0 group-hover:opacity-100 transition-all">
                    <button onClick={handleSaveToHistory} disabled={saving || isSaved} className="px-8 py-4 min-h-[44px] rounded-full bg-secondary text-white text-[10px] font-black uppercase tracking-widest shadow-[0_18px_50px_-28px_rgba(0,0,0,0.7)] hover:brightness-110">
                      {isSaved ? 'Archived' : 'Curate to Gallery'}
                    </button>
                    <button onClick={() => setGeneratedImage(null)} className="px-8 py-4 min-h-[44px] rounded-full bg-black/60 backdrop-blur-sm text-white text-[10px] font-black uppercase tracking-widest">
                      Discard
                    </button>
                  </div>
                </>
      ...
      {history.length > 0 && (
        <ScrollReveal direction="up" className="space-y-12">
          <div className="flex items-center gap-6">
             <h3 className="text-2xl font-display font-bold text-white uppercase tracking-widest">Neural <span className="gradient-text">Gallery</span></h3>
             <div className="h-px flex-grow bg-white/10" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {history.map((item, i) => (
              <div key={i} className="glass-card rounded-[2rem] overflow-hidden aspect-square reactive-glass group border-white/10">
                <img 
                  src={item.imageData} 
                ...
                <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-end p-4">
                  ...
                  <button 
                    ...
                    className="w-full py-2 min-h-[44px] rounded-xl bg-white/20 text-[8px] font-bold uppercase text-white hover:bg-white/30"
                  >
                    View Original
                  </button>
                </div>
